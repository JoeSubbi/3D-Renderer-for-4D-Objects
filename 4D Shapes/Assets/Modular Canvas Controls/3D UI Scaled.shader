    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                // object space
                o.ro = mul(unity_WorldToObject, _WorldSpaceCameraPos + float3(_X,_Y+1.2,_Z));
                o.hitPos = v.vertex;
                return o;
            }
            ...
            float Shape(float3 p){
                if (_Shape == 1){
                    p -= float3(0,0.4,0);
                    p = Rotate(p);
                    return sdBox(p, float3(0.7,0.7,0.7))-0.005;
                }
                if (_Shape == 2){
                    p -= float3(0,0.4,0);
                    p = Rotate(p);
